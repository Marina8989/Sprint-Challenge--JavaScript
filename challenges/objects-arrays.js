...
for (let i = 0; i < graduates.length; i++) {
    let newArr = graduates[i].university;
    universities.push(newArr);
    universities.sort();
}
graduates.forEach((univ, index) => {
...
for (i = 0; i < graduates.length; i++) {
    let newArray = `${graduates[i].first_name} ${graduates[i].email}`;
    contactInfo.push(newArray);
}
graduates.forEach((str, index) => {
...
// const unisWithUni = [];
const str = 'Uni';
const unisWithUni = graduates.filter((arr) => {
    return arr.university.includes(str);
});
...
for (i = 0; i < zooAnimals.length; i++) {
    let zName = ` Name: ${zooAnimals[i].animal_name}, Scientific: ${zooAnimals[i].scientific_name}.`;
    displayNames.push(zName);
}

zooAnimals.forEach((animalName, index) => {
...
const lowCaseAnimalNames = zooAnimals.map((arr) => {
...
*/
const lowPopulationAnimals = zooAnimals.filter((count) => {
...
*/
const populationTotal = zooAnimals.reduce((total, count) => {
    return total += count.population;
}, 0);
